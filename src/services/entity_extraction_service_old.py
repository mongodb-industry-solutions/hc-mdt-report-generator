...
class EntityExtractionService:  
    ...
    def __init__(self,   
        ...
        self.continue_on_errors = getattr(ner_settings, 'continue_on_batch_errors', True)
    ...
    async def extract_entities_from_documents(  
        ...
        try:  
            ...
            for processing_type, entity_list in entities_by_type.items():  
                ...
                # Report progress at start of each processing type
                if progress_callback:
                    progress = 5 + (current_type_index / total_types) * 90  # 5-95% range
    ...
    async def _extract_entities_by_type_robust(  
        ...
        async def report_document_progress(doc_index: int, message: str):
            ...
            async with progress_lock:
                ...
                progress = 5 + ((current_type_index + progress_within_type * 0.8) / total_types) * 90
        ...
        if processing_type == ProcessingType.MULTIPLE_MATCH:  
            ...
            try:
                ...
                if progress_callback:
                    progress = 5 + ((current_type_index + 0.8) / total_types) * 90
                    await progress_callback({
                        "status": "EXTRACTING_ENTITIES",
                        "progress": int(progress),
